...
// File: samples/maps/projimg/src/centeri.cpp
// --------------------------------------------------------------------------
//
...
// --------------------------------------------------------------------------
CenterInteractor::CenterInteractor(IlvManager* manager,IlvView* view)
      ...
      _currentCursor(0)
{
    ...
    mask = new IlvBitmap(display,
			 ...
			 8);
    if (bm && mask) {
	_centerCursor = new IlvCursor(display, 16, 16, "centerCursor",
				      bm, mask);
    }
    if (_centerCursor) {
	_centerCursor->lock();
    ...
    mask = new IlvBitmap(display,
			 ...
			 8);
    if (bm && mask) {
	_forbidCursor = new IlvCursor(display, 17, 17, "forbidCursor",
				      bm, mask);
    }
    if (_forbidCursor) {
	_forbidCursor->lock();
...
// --------------------------------------------------------------------------
void
CenterInteractor::doIt(IlvEvent& event)
{
}
