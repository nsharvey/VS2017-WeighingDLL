...
// File: samples/charts/simple/src/simple.cpp
// --------------------------------------------------------------------------
//
...
{
    IlvSetLocale();
    IlvDisplay* display = new IlvDisplay("SimpleApp", "", argc, argv);
    if (display->isBad()) {
    ...
    localPath.addDirectory("samples/charts/simple/data");
    display->prependToPath(localPath);

    const char* title = "Rogue Wave Views sample: simple charts";
