// -------------------------------------------------------------- -*- C++ -*-
//						 Rogue Wave Views source file
// File: include/ilviews/maps/geometry/geosegstr.h
// --------------------------------------------------------------------------
//
// Licensed Materials - Property of Rogue Wave Software, Inc.
// (c) Copyright Rogue Wave Software, Inc. 2012, 2018
// (c) Copyright IBM Corp. 2009, 2011
// (c) Copyright ILOG 1992, 2009
// All Rights Reserved.
//
// Note to U.S. Government Users Restricted Rights:
// The Software and Documentation were developed at private expense and
// are "Commercial Items" as that term is defined at 48 CFR 2.101,
// consisting of "Commercial Computer Software" and
// "Commercial Computer Software Documentation", as such terms are
// used in 48 CFR 12.212 or 48 CFR 227.7202-1 through 227.7202-4,
// as applicable.
//
// --------------------------------------------------------------------------
// Rogue Wave Views - Maps add-on.
// Defines an interface common to all strings
// --------------------------------------------------------------------------

#ifndef _Ilv_Maps_String_H
#define _Ilv_Maps_String_H

#include <ilviews/maps/macros.h>
#include <ilviews/maps/geometry/geosegment.h>
#include <ilviews/base/clssinfo.h>

ILVMAPSMODULEINIT(IlvMapSegmentString);


class ILVMAPSEXPORTED IlvMapSegmentString
    :public IlvMapSegment
{
protected:
    IlvMapSegmentString();

public:
    virtual ~IlvMapSegmentString();

    virtual IlUInt getSegmentCount() const = 0;

    virtual const IlvMapSegment* getSegment(IlUInt index) const = 0;

    virtual void removeAll() = 0;

    virtual const IlvCoordinate& getStartPoint() const = 0;

    virtual const IlvCoordinate& getEndPoint() const = 0;

    virtual void setStartPoint(const IlvCoordinate& start) = 0;

    virtual void setEndPoint(const IlvCoordinate& end) = 0;

    // documented in the superclass
    virtual IlBoolean isCollection() const;
    virtual IlUInt getSubElementCount() const;
    virtual const IlvMapGeometry* getSubElement(IlUInt index) const;
    virtual IlvMapGeometry* copy() const = 0;
    virtual IlvClassInfo* getClassInfo() const = 0;

    ILVMAPS_DECL_GEOM_MEMBERS();
private:
	IlvMapsIcFriend(IlvMapSegmentString, ILVMAPSEXPORTED);
};

#endif /* _Ilv_Maps_String_H */

