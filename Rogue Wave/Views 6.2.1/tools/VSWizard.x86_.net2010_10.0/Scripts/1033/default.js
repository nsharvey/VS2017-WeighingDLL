...
function AddConfig(proj, strProjectName)
{
	...
	var useOfATLstatic = wizard.FindSymbol('ATL_STATIC_LIB');
	var useOfATLshared = wizard.FindSymbol('ATL_SHARED_LIB');
	...
	if (useOLE) {
	    otherLibs += " oledlg.lib";
	    if (useOfATLstatic) {
		otherLibs += " ilvcom.lib";
	    } else if (useOfATLshared) {
		otherLibs += " ilvcom.lib";
	    }
	}
	if (useBMP) {
	...
	if (useOLE) {
	    if (useOfATLstatic) {
		config_debug.useOfATL = useOfATL.useATLStatic;
		config_release.useOfATL = useOfATL.useATLStatic;
	    }
	    else if (useOfATLshared) {
		config_debug.useOfATL = useOfATL.useATLDynamic;
		config_release.useOfATL = useOfATL.useATLDynamic;
	    }
	}
	...
	var libraryDir = ".;" + strILVHOME + "lib\\x86_.net2013_12.0\\" + strLibFormat;
	if (strIFCHOME) {
	    libraryDir += ";" + strIFCHOME + "lib\\x86_.net2013_12.0\\" + strLibFormat;
	}
