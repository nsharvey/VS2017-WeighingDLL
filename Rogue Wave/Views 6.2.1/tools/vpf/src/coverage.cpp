...
// File: tools/vpf/src/coverage.cpp
// --------------------------------------------------------------------------
//
...
#if 0
static const char* joinTypes[] = { ".pft",".aft", ".tft", ".lft",
				   ".ajt", ".ljt", ".rjt", ".njt", 0};
#else
...
VpfCoverage::initFeatureClasses()
{
	...
	goto error;
    if (((fcsCursor = new VpfFcsCursor(fcsTable)) == 0) || fcsCursor->isBad())
	goto error;
    if (((fcsRow = (VpfFcsRow*)fcsCursor->getNewRow()) == 0) ||
	fcsRow->isBad())
	goto error;
    if (((fcsRow2 = (VpfFcsRow*)fcsCursor->getNewRow()) == 0) ||
	fcsRow2->isBad())
	goto error;
	...
	for (i = 0; i < nFcsRows; i++) {
	    if (fcNames[i]) {
		for (VpfUInt j = i + 1; j < nFcsRows; j++) {
	...
	for (i = 0; i < _nFeatureClasses; i++) {
		...
		while (!fcaCursor->getNextRow(fcaRow))
...
VpfCoverage::initThemes()
{
    ...
    VpfErrorReporter::Push();
    VpfTable* cvdtTable = new VpfTable(cvdt);
    VpfErrorReporter::Pop();
    ...
    VpfErrorReporter::Push();
    VpfTable* ivdtTable = new VpfTable(ivdt);
    VpfErrorReporter::Pop();
