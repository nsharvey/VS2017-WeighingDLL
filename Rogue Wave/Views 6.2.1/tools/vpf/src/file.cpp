		  ...
		  VpfUInt count, int trailingNull)
{
	...
	VpfError* error = new VpfError("VpfFile::readText");
...
VpfFile::readInteger(VpfInt* intValue, char type, VpfUInt count)
{
	...
	if (::fread((char*)intValue, sizeof(VpfInt), count, _file) != count) {
	    VpfError* error = new VpfError("VpfFile::readInteger");
	    error->getStream() << "A file error occured.";
	    VpfErrorReporter::Add(error);
	    returnCode = 1;
	} else
    ...
    default: {
	VpfError* error = new VpfError("VpfCursor::readInteger");
...
VpfFile::readFloat(double* doubleValue, char type, VpfUInt count)
{
    ...
    switch (type) {
    case 'L': {
	if (::fread((char*)doubleValue, sizeof(double),
		    count, _file) != count) {
	    VpfError* error = new VpfError("VpfFile::readFloat");
	    error->getStream() << "A file error occured.";
	    VpfErrorReporter::Add(error);
	    returnCode = 1;
	} else
...
VpfFile::readTripletPart(int size, VpfInt& value)
{
	...
	if (::fread(tmp, 1, 1, _file) != 1) {
	    VpfError* error = new VpfError("VpfFile::readTripletPart");
	    error->getStream() << "A file error occured.";
	    VpfErrorReporter::Add(error);
	    returnCode = 1;
	} else
...
VpfFile::readTripletID(VpfTripletID* triplet, VpfUInt count)
{
	...
	VpfError* error = new VpfError("VpfFile::readTripletID");
	error->getStream() << "A file error occured.";
	VpfErrorReporter::Add(error);
	returnCode = 1;
    } else {
...
VpfFile::readCoordinate(VpfCoordinate* coordinates, char type, VpfUInt count)
{
	...
	for (VpfUInt i = 0; i < count; i++) {
		...
		VpfError* error = new VpfError("VpfFile::readCoordinate");
		error->getStream() << "A file error occured.";
		VpfErrorReporter::Add(error);
		returnCode = 1;
	    }
