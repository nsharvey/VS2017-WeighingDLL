...
VpfTable::parseDataDefinition(VpfFile* tableFile)
{
    ...
    p = 2;
    const char* stringGot = GetString(p, buf, ComponentSeparator);
	...
	_headers[i]->setType((char)toupper(GetChar(p, buf)));
	...
	if (buf[p] == EndOfField) {
	    // VpfError("VpfTable::parseDataDef: old table definition for < %s >", _name);
	}
...
VpfTable::CopyToDelimiter(VpfUInt& ind, char* src, char delimiter)
{
    ...
    skipchar = 0 ;
    while ((src[skipchar] == ' ') || (src[skipchar] == '\t'))
	skipchar++ ;
...
// --------------------------------------------------------------------------
void
VpfTable::dump(ostream& out, long start, long end) const
{
    if (start <= 0)
	start = 1;
    else if (VpfUInt(start) >= getNRows())
	start = getNRows();
    if (end <= 0)
	end = getNRows();
    else if (VpfUInt(end) >= getNRows())
	end = getNRows();
    if (end < start) {
	int t = end;
	end = start;
    ...
    out << "Table: '" << getName() << "' ["
	<< getNFields() << " fields, " << getNRows() << " rows]" << endl
	<< "Description: '" << getDescription() << "'" << endl;
    VpfHeader** headers = getHeaders();
    VpfUInt loop;
    for (loop = 0; loop < getNFields(); loop++) {
	out << (loop+1) << ": '" << (*headers)->getName() << "' ("
	    << (*headers)->getType() << ",";
	if ((*headers)->getCount() < 0)
	    out << "*";
	else
	    out << (*headers)->getCount();
	out << "): " << (*headers)->getDescription() << endl;
	headers++;
    }
    VpfCursor cursor(this);
    if (!cursor.isBad()) {
	out << "----------------------------------------" << endl;
	VpfRow* row = cursor.getNewRow();
	cursor.setCurrentRow((VpfUInt)(start-1));
	VpfUInt count;
	for (loop = (VpfUInt)start; loop <= (VpfUInt)end; loop++) {
	    cursor.getNextRow(row);
	    out << "Row " << row->getRowNumber() << endl;
	    for (VpfUInt h = 0; h < getNFields(); h++) {
		out << "    " << _headers[h]->getName() << "=";
		count = row->getCount(h);
		if (count != 1)
		    out << "[" << count << "] ";
		switch (_headers[h]->getType()) {
		case 'I': {
		    VpfInt i;
		    for (VpfUInt l = 0; l < count; l++) {
			row->getIntField(h, i, l);
			if (l)
			    out << ", ";
			out << i;
		    }
