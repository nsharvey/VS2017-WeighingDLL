...
// File: tools/vpf/src/areafeature.cpp
// --------------------------------------------------------------------------
//
...
VpfAreaFeature::selectTile(VpfInt tileID)
{
    ...
    if (((_faceTable = new VpfTable(buffer1)) == 0) || _faceTable->isBad())
	return 1;
    if (((_faceCursor = new VpfFacCursor(_faceTable)) == 0) ||
	_faceCursor->isBad())
	return 1;
    if (((_faceRow =(VpfFacRow*)_faceCursor->getNewRow()) == 0) ||
	_faceRow->isBad())
	return 1;

    // Open the edge table with a level 3 topology
    // since it is not the default
    if (((_edgeTable = new VpfTable(buffer2)) == 0) || _edgeTable->isBad() ||
	((_edgeCursor = new VpfEdgCursor(_edgeTable, 3)) == 0) ||
	_edgeCursor->isBad() ||
	((_edgeRow = (VpfEdgRow*)_edgeCursor->getNewRow()) == 0) ||
	_edgeRow->isBad())
	return 1;
    else
...
VpfAreaFeature::getRingCoords(VpfInt id, VpfInt faceID, VpfInt startEdge)
{
	...
	VpfSegment* segment = new VpfSegment(n + 1, nPoints,
...
VpfAreaFeature::getFeature(int primitiveID)
{
    ...
    VpfArea* area = new VpfArea(primitiveID, VPFMAXRINGS);
    VpfRing* ring = getRingCoords(n + 1, primitiveID,
