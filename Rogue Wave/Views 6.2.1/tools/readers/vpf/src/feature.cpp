...
// File: tools/readers/vpf/src/feature.cpp
// --------------------------------------------------------------------------
//
...
// --------------------------------------------------------------------------
int
VpfFeature::select(const char* expression)
{
    if (!_featureTable) {
	char* buffer;
    ...
    VpfQuery* query = new VpfQuery(_featureTable, expression);
    int result = _featureCursor->query(query);
    delete query;
    _currentTileID = -1;
    _layerIndex = 0;
...
VpfFeature::getNextObject()
{
	...
	if (reshape && item) {
	    reshapedItem = item->reshape(extent);
