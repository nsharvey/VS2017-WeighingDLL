...
// File: tools/readers/vpf/src/database.cpp
// --------------------------------------------------------------------------
//
  ...
  _libraries(0)
{
    ...
    _version = VpfStrDup(dhtRow->getVersion());
    _name = VpfStrDup(dhtRow->getDatabaseName());
    _description = VpfStrDup(dhtRow->getDatabaseDesc());
    _mediaStandard = VpfStrDup(dhtRow->getMediaStandard());
    _originator = VpfStrDup(dhtRow->getOriginator());
    _addressee = VpfStrDup(dhtRow->getAddressee());
    _mediaVolumes = VpfStrDup(dhtRow->getMediaVolumes());
    _sequenceNumbers = VpfStrDup(dhtRow->getSeqNumbers());
    _numberOfDataSets = VpfStrDup(dhtRow->getNumDataSets());
    _securityClass = dhtRow->getSecurityClass();
    _downgrading = VpfStrDup(dhtRow->getDowngrading());
    
    _downgradeDate = dhtRow->getDowngradeDate();
    if (_downgradeDate)
	_downgradeDate = new VpfDate(*_downgradeDate);

    _releasability = VpfStrDup(dhtRow->getReleasability());
    _otherStandardName = VpfStrDup(dhtRow->getOtherStdName());

    _otherStandardDate = dhtRow->getOtherStdDate();
    if (_otherStandardDate)
	_otherStandardDate = new VpfDate(*_otherStandardDate);

    _otherStandardVersion = VpfStrDup(dhtRow->getOtherStdVer());
    _transmittalID = VpfStrDup(dhtRow->getTransmittalID());
    _editionNumber = VpfStrDup(dhtRow->getEditionNumber());
	...
	libraryName = latRow->getLibraryName();
	xmin = latRow->getXMin();
	ymin = latRow->getYMin();
	xmax = latRow->getXMax();
	ymax = latRow->getYMax();
    ...
    delete latRow; latRow = 0;
    delete latCursor; latCursor = 0;
    delete latTable; latTable = 0;
