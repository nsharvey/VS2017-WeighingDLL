...
// File: doc/tutorials/foundation/ActiveXGraphic/step6/specific.cpp
// --------------------------------------------------------------------------
//
...
MakeNodeInteractor::doIt(IlvRect& size)
{
    ...
    // Coded from the step5.
    IlvControlBrowser cBrowser(getView()->getSystemView());
    IlvRect bbox;
    getView()->globalBBox(bbox);
    const char* identifier = 0; //cBrowser.show(bbox.centerx(), bbox.centery());
    if (!identifier)
