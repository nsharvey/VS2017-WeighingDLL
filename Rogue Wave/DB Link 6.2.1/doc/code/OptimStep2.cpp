...
void Ending(IldDbms* dbms)
{
    ...
    const char* dropStr = "drop table OPTIMS2";
...
void runDisplay(IldDbms* dbms)
{
    ...
    const char* selectStr = "select I, S from OPTIMS2";
...
// ----------------------------------------------------------------------------
// This part will use an array of parameters to send several insert queries
// to the RDBMS at once.
void runStep1(IldDbms* dbms)
{
    IldRequest* request = dbms->getFreeRequest();
    if (dbms->isErrorRaised()) {
	...
	insertStr = "insert into OPTIMS2 values (:1, :2)";
    else
	insertStr = "insert into OPTIMS2 values (?, ?)";
	 ...
	 char* argv[])
{
    ...
    // Create a new table.
    const char* createStr = "create table OPTIMS2(I int, S char(20))";
