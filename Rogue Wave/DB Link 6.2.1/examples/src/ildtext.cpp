...
//       will be retireved using piece-wise fetch.
//
// ----------------------------------------------------------------------------
     ...
     char** argv)
{
    ...
    IldDisplayHeader("Long text handling");
	...
	cout << "Asynchronous mode is turned ON." << endl;
    }
    // Table creation.
    char lobType [128];
    ostrstream ostr(lobType, 128);
    ostr << IldGetLongTextTypeName(dbms)
	 << ((!strcmp(argv[1], "sybase") ||
	      !strcmp(argv[1], "mssql")) ? " null" : "") << ends;
